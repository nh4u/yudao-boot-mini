...
public class ModbusTagReader {
    ...
    // 连接配置
    private static final String HOST = "192.168.0.10";  // ← 请改为你的设备 IP
    private static final int PORT = Modbus.DEFAULT_PORT; // 通常是 502
    private static final int UNIT_ID = 5;                // 从站编号
  ...
  //  private static final int INTERVAL_MINUTES = 1;       // 每次间隔
    ...
    private static final String EXCEL_FILE_PATH = "D:\\全是文档\\燕东\\施耐德.xlsx";
    ...
    /*    private static final LinkedHashMap<String, Integer> TAG_ADDRESS_MAP = new LinkedHashMap<>();
        static {
             TAG_ADDRESS_MAP.put("LCR-电机电流", 43204);
             TAG_ADDRESS_MAP.put("NPR-电机额定功率", 49613);
             TAG_ADDRESS_MAP.put("UOP-电机电压-V", 43208);
             TAG_ADDRESS_MAP.put("OPR-电机功率-%", 43211);
             TAG_ADDRESS_MAP.put("OC0-电机消耗的电能-Wh", 41606);
             TAG_ADDRESS_MAP.put("OC1-电机消耗的电能-KWh", 410607);
             TAG_ADDRESS_MAP.put("OC2-电机消耗的电能-MWh", 410608);
             TAG_ADDRESS_MAP.put("OC3-电机消耗的电能-GWh", 410609);
             TAG_ADDRESS_MAP.put("OC4-电机消耗的电能-TWh", 410610);
             TAG_ADDRESS_MAP.put("OP0-电机产生的电能-WH", 410611);
             TAG_ADDRESS_MAP.put("OP1-电机产生的电能-KWh",410612);
             TAG_ADDRESS_MAP.put("OP2-电机产生的电能-MWh",410613);
             TAG_ADDRESS_MAP.put("OP3-电机产生的电能-GWh",410614);
             TAG_ADDRESS_MAP.put("OP4-电机产生的电能-TWh",410615);
             TAG_ADDRESS_MAP.put("OE0-实际能耗-WH",  410616);
             TAG_ADDRESS_MAP.put("OE1-实际能耗-KWh", 410617);
             TAG_ADDRESS_MAP.put("OE2-实际能耗-MWh", 410618);
             TAG_ADDRESS_MAP.put("OE3-实际能耗-GWh", 410619);
             TAG_ADDRESS_MAP.put("OE4-实际能耗-TWh", 410620);
        }*/
    ...
    public static void main(String[] args) throws Exception {
       // ModbusSlave modbusTcpServer = ModbusSlaveFullExample.createModbusTcpServer();
        ...
        int col = 1;
        for (Map.Entry<String, Integer> entry : TAG_ADDRESS_MAP.entrySet()) {
            header.createCell(col++).setCellValue(entry.getKey() + "(" + entry.getValue() + ")");
        }
        ...
        scheduler.scheduleAtFixedRate(() -> {
            ...
            if (runCount[0] >= TOTAL_DURATION_MINUTES) {
                scheduler.shutdown();
                saveExcel();
                System.out.println("任务完成，数据已保存至: " + EXCEL_FILE_PATH);
            }
    ...
    // 写入一行 Excel
    private static void writeExcelRow(List<Integer> values) {
    ...
    private static void saveExcel() {
        try (FileOutputStream out = new FileOutputStream(EXCEL_FILE_PATH)) {
            workbook.write(out);
            workbook.close();
        } catch (Exception e) {
            System.err.println("保存 Excel 出错: " + e.getMessage());
        }
    }
}
