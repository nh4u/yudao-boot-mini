...
public class CornUtils {
     ...
     * @return cron表达式
     */
    public static String getCorn(Long frequency, Integer frequencyUnit) {
        ...
        switch (acquisitionFrequencyUnit) {
            case SECONDS: // 秒
                corn = String.format("0/%d * * * * ?", frequency);
                break;
            case MINUTES: // 分钟
                corn = String.format("0 0/%d * * * ?", frequency);
                break;
            case HOUR: // 小时
                corn = String.format("0 0 0/%d * * ?", frequency);
                break;
            case DAY: // 天
                corn = String.format("0 0 0 0/%d * ?", frequency);
                break;
            default:
