...
import cn.bitlinks.ems.module.power.controller.admin.warninginfo.vo.WarningInfoGroupDTO;
...
public interface WarningInfoMapper extends BaseMapperX<WarningInfoDO> {
    ...
    int updateStatusAndOpinionByGroups(@Param("status") Integer status,
    ...
    @Select("SELECT " +
        ...
        "SUM(CASE WHEN wi.level = 4 THEN 1 ELSE 0 END) AS count4 " +
        "FROM power_warning_info wi " +
        "WHERE wi.device_rel like concat('%(', #{sbCode}, ')%')")
    WarningInfoStatisticsRespVO getMonitorStatisticsBySbCode(@Param("sbCode") String sbCode);
}