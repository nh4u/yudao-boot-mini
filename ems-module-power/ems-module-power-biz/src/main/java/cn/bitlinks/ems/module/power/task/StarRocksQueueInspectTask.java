...
public class StarRocksQueueInspectTask {
    ...
    public void startInspectThread() {
        new Thread(() -> {
            while (true) {
                try {
                    // 检查 quick=true 队列
                    inspectQueue(Boolean.TRUE);
                    // 检查 quick=false 队列
                    inspectQueue(Boolean.FALSE);
                    // 巡检间隔：5秒
    ...
    // 单个队列的巡检逻辑
    private void inspectQueue(Boolean quick) {
        // 1. 获取队列配置（复用StarRocksBatchImportService的方法，需改为public或提供工具方法）
        String queueKey = starRocksBatchImportService.getQueueKeyByAcq(quick);
        int threshold = starRocksBatchImportService.getThresholdByAcq(quick);
        ...
        try {
            if (!lock.tryLock(3000L, TimeUnit.MILLISECONDS)) {
                log.warn("quick={} 其他实例正在巡检，跳过", quick);
                return;
            }
            Long queueSize = redisTemplate.opsForList().size(redisKey);
            // 检查条件：队列有数据且≥阈值
            if (queueSize != null && queueSize >= threshold) {
                log.info("巡检发现 quick={} 队列够阈值（当前：{}，阈值：{}），触发导入",
                        quick, queueSize, threshold);
                starRocksBatchImportService.triggerBatchImport(quick);
            } else {
                log.debug("巡检 quick={} 队列：长度{}（不足阈值{}），跳过",
                        quick, queueSize, threshold);
            }
        } catch (Exception e) {
    ...
    public void flushQueuePeriodicallyQuick() {
        String lockKey = String.format(STARROCKS_FLUSH_LOCK_KEY, env, true);
        RLock lock = redissonClient.getLock(lockKey);
        try {
            if (!lock.tryLock(3000L, TimeUnit.MILLISECONDS)) {
            ...
            starRocksBatchImportService.flushQueuePeriodically(true);
        } catch (Exception e) {
    ...
    /**
     * acq=0 定时兜底：每10分钟执行（减少调用）
     */
    @Scheduled(fixedRate = FLUSH_INTERVAL)
    public void flushQueuePeriodically() {
        String lockKey = String.format(STARROCKS_FLUSH_LOCK_KEY, env, false);
        RLock lock = redissonClient.getLock(lockKey);
        try {
            if (!lock.tryLock(3000L, TimeUnit.MILLISECONDS)) {
                log.warn("starocks批量慢队列不满阈值触发 其他实例进行中，跳过");
                return;
            }
            starRocksBatchImportService.flushQueuePeriodically(false);
        } catch (Exception e) {
            log.error("starocks批量慢队列不满阈值触发 发生异常", e);
        } finally {
            if (lock.isHeldByCurrentThread()) {
                lock.unlock();
            }
