...
public class StandingbookAttributeServiceImpl implements StandingbookAttributeService {
    ...
    public List<StandingbookAttributeDO> getPrivateStandingbookAttributeByTypeId(Long typeId) {
        return standingbookAttributeMapper.selectList(
                new LambdaQueryWrapper<StandingbookAttributeDO>()
                        .eq(StandingbookAttributeDO::getTypeId, typeId)
                        .isNull(StandingbookAttributeDO::getStandingbookId)
    ...
    public void saveMultiple(List<StandingbookAttributeSaveReqVO> createReqVOs) {
        ...
        // 当剩余最后一个自定义属性删除时 也要给删除，即：optAttrList为空时
        Long typeId;
        if (CollUtil.isEmpty(optAttrList)) {
            typeId = createReqVOs.get(0).getTypeId();
        }else {
            typeId = optAttrList.get(0).getTypeId();
        }
        ...
        // 找出当前属性列表
        List<StandingbookAttributeDO> rawAttrList = getPrivateStandingbookAttributeByTypeId(typeId);
        // 级联删除
     ...
     */
    public boolean checkModifiedSortAttributes(List<StandingbookAttributeDO> updateList,
                                               List<StandingbookAttributeDO> rawAttrList) {
        return updateList.stream()
                .allMatch(attr -> rawAttrList.stream()
                        .anyMatch(rawAttr -> attr.getId().equals(rawAttr.getId()) &&
                                Objects.equals(attr.getName(), rawAttr.getName()) &&
                                Objects.equals(attr.getFormat(), rawAttr.getFormat()) &&
                                Objects.equals(attr.getIsRequired(), rawAttr.getIsRequired()) &&
                                !Objects.equals(attr.getSort(), rawAttr.getSort())));
    ...
    public static void filterModifiedAttributes(List<StandingbookAttributeDO> updateList, List<StandingbookAttributeDO> rawAttrList) {
        updateList.removeIf(updatedAttr -> rawAttrList.stream()
                .anyMatch(rawAttr -> updatedAttr.getId().equals(rawAttr.getId()) &&
                        Objects.equals(updatedAttr.getName(), rawAttr.getName()) &&
    ...
    public void updateAttrCascade(List<StandingbookAttributeDO> updateList, List<StandingbookAttributeDO> rawAttrList
        ...
        // 0.1 判断是否只更改了sort属性,
        boolean onlySortChanged = checkModifiedSortAttributes(updateList, rawAttrList);
