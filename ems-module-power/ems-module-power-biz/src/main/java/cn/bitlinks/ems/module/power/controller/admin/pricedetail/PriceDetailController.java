...
public class PriceDetailController {
    ...
    @PostMapping("/create")
    @Operation(summary = "创建单价详细")
    @PreAuthorize("@ss.hasPermission('power:price-detail:create')")
    public CommonResult<Long> createPriceDetail(@Valid @RequestBody PriceDetailSaveReqVO createReqVO) {
        return success(priceDetailService.createPriceDetail(createReqVO));
    }

    @PutMapping("/update")
    @Operation(summary = "更新单价详细")
    @PreAuthorize("@ss.hasPermission('power:price-detail:update')")
    public CommonResult<Boolean> updatePriceDetail(@Valid @RequestBody PriceDetailSaveReqVO updateReqVO) {
        priceDetailService.updatePriceDetail(updateReqVO);
        return success(true);
    }
