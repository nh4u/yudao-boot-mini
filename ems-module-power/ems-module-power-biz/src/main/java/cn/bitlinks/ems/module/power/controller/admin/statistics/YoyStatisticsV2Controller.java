...
@Tag(name = "管理后台 - 用能分析/同比分析")
@RestController
@RequestMapping("/power/statistics/yoy/v2")
@Validated
public class YoyStatisticsV2Controller {
    ...
    public void exportDiscountAnalysisTable(@Valid @RequestBody StatisticsParamV2VO paramVO,
        ...
        switch (queryType) {
            case 0:
                filename = COST_YOY_ALL + XLSX;
                mergeIndex = labelDeep;
                break;
            case 1:
                filename = COST_YOY_ENERGY + XLSX;
                // 能源不需要合并
                mergeIndex = 0;
                break;
            case 2:
                filename = COST_YOY_LABEL + XLSX;
                // 标签没有能源
...
//                .registerWriteHandler(new MatchTitleWidthStyleStrategy())
    ...
    public void exportFoldCoalAnalysisTable(@Valid @RequestBody StatisticsParamV2VO paramVO,
        ...
        switch (queryType) {
            case 0:
                filename = STANDARD_COAL_YOY_ALL + XLSX;
                mergeIndex = labelDeep;
                break;
            case 1:
                filename = STANDARD_COAL_YOY_ENERGY + XLSX;
                // 能源不需要合并
                mergeIndex = 0;
                break;
            case 2:
                filename = STANDARD_COAL_YOY_LABEL + XLSX;
                // 标签没有能源
                mergeIndex = labelDeep - 1;
                break;
            default:
