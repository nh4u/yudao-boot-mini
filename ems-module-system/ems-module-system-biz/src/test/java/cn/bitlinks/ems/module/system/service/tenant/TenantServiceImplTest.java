package cn.bitlinks.ems.module.system.service.tenant;

import cn.bitlinks.ems.framework.common.enums.CommonStatusEnum;
import cn.bitlinks.ems.framework.common.pojo.PageResult;
import cn.bitlinks.ems.framework.tenant.config.TenantProperties;
...
import cn.bitlinks.ems.module.system.controller.admin.tenant.vo.tenant.TenantPageReqVO;
import cn.bitlinks.ems.module.system.controller.admin.tenant.vo.tenant.TenantSaveReqVO;
import cn.bitlinks.ems.module.system.dal.dataobject.permission.MenuDO;
import cn.bitlinks.ems.module.system.dal.dataobject.permission.RoleDO;
import cn.bitlinks.ems.module.system.dal.dataobject.tenant.TenantDO;
import cn.bitlinks.ems.module.system.dal.dataobject.tenant.TenantPackageDO;
import cn.bitlinks.ems.module.system.dal.mysql.tenant.TenantMapper;
import cn.bitlinks.ems.module.system.enums.permission.RoleCodeEnum;
import cn.bitlinks.ems.module.system.enums.permission.RoleTypeEnum;
import cn.bitlinks.ems.module.system.service.permission.MenuService;
...
import cn.bitlinks.ems.module.system.service.tenant.handler.TenantInfoHandler;
import cn.bitlinks.ems.module.system.service.tenant.handler.TenantMenuHandler;
...
import static cn.bitlinks.ems.framework.common.util.collection.SetUtils.asSet;
...
import static cn.bitlinks.ems.module.system.dal.dataobject.tenant.TenantDO.PACKAGE_ID_SYSTEM;
...
public class TenantServiceImplTest extends BaseDbUnitTest {
    ...
    public void testCreateTenant() {
        ...
        when(userService.createUser(argThat(user -> {
            assertEquals("bitlinks", user.getUsername());
            assertEquals("bitlinks", user.getPassword());
            assertEquals("bitlinks", user.getNickname());
        ...
        TenantSaveReqVO reqVO = randomPojo(TenantSaveReqVO.class, o -> {
            o.setContactName("bitlinks");
            o.setContactMobile("15601691300");
            o.setPackageId(100L);
            o.setStatus(randomCommonStatus());
            o.setWebsite("https://www.bitlinks.cn");
            o.setUsername("bitlinks");
            o.setPassword("bitlinks");
    ...
    public void testGetTenantByName() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setName("bitlinks"));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据

        // 调用
        TenantDO result = tenantService.getTenantByName("bitlinks");
        // 校验存在
        assertPojoEquals(result, dbTenant);
    }
    ...
    public void testGetTenantByWebsite() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setWebsite("https://www.bitlinks.cn"));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据

        // 调用
        TenantDO result = tenantService.getTenantByWebsite("https://www.bitlinks.cn");
        // 校验存在
        assertPojoEquals(result, dbTenant);
    }
