package cn.bitlinks.ems.module.infra.service.file;
...
import cn.bitlinks.ems.module.infra.controller.admin.file.vo.config.FileConfigPageReqVO;
import cn.bitlinks.ems.module.infra.controller.admin.file.vo.config.FileConfigSaveReqVO;
import cn.bitlinks.ems.module.infra.dal.dataobject.file.FileConfigDO;
import cn.bitlinks.ems.module.infra.dal.mysql.file.FileConfigMapper;
import cn.bitlinks.ems.module.infra.framework.file.core.client.FileClient;
import cn.bitlinks.ems.module.infra.framework.file.core.client.FileClientConfig;
import cn.bitlinks.ems.module.infra.framework.file.core.client.FileClientFactory;
import cn.bitlinks.ems.module.infra.framework.file.core.client.local.LocalFileClient;
import cn.bitlinks.ems.module.infra.framework.file.core.client.local.LocalFileClientConfig;
import cn.bitlinks.ems.module.infra.framework.file.core.enums.FileStorageEnum;
import lombok.Data;
...
import static cn.bitlinks.ems.framework.test.core.util.RandomUtils.randomLongId;
import static cn.bitlinks.ems.framework.test.core.util.RandomUtils.randomPojo;
import static cn.bitlinks.ems.module.infra.enums.ErrorCodeConstants.FILE_CONFIG_DELETE_FAIL_MASTER;
import static cn.bitlinks.ems.module.infra.enums.ErrorCodeConstants.FILE_CONFIG_NOT_EXISTS;
...
public class FileConfigServiceImplTest extends BaseDbUnitTest {
    ...
    public void testCreateFileConfig_success() {
        // 准备参数
        Map<String, Object> config = MapUtil.<String, Object>builder().put("basePath", "/bitlinks")
                .put("domain", "https://www.bitlinks.cn").build();
        ...
        assertEquals("/bitlinks", ((LocalFileClientConfig) fileConfig.getConfig()).getBasePath());
        assertEquals("https://www.bitlinks.cn", ((LocalFileClientConfig) fileConfig.getConfig()).getDomain());
        // 验证 cache
    ...
    public void testUpdateFileConfig_success() {
        ...
        FileConfigDO dbFileConfig = randomPojo(FileConfigDO.class, o -> o.setStorage(FileStorageEnum.LOCAL.getStorage())
                .setConfig(new LocalFileClientConfig().setBasePath("/bitlinks").setDomain("https://www.bitlinks.cn")));
        ...
        FileConfigSaveReqVO reqVO = randomPojo(FileConfigSaveReqVO.class, o -> {
            ...
            Map<String, Object> config = MapUtil.<String, Object>builder().put("basePath", "/bitlinks2")
                    .put("domain", "https://doc.bitlinks.cn").build();
            o.setConfig(config);
        });
        ...
        assertEquals("/bitlinks2", ((LocalFileClientConfig) fileConfig.getConfig()).getBasePath());
        assertEquals("https://doc.bitlinks.cn", ((LocalFileClientConfig) fileConfig.getConfig()).getDomain());
        // 验证 cache
    ...
    public void testGetFileConfigPage() {
        // mock 数据
        FileConfigDO dbFileConfig = randomFileConfigDO().setName("bitlinks")
    ...
    public void testFileConfig() throws Exception {
        ...
        when(fileClient.upload(any(), any(), any())).thenReturn("https://www.bitlinks.cn");

        // 调用，并断言
        assertEquals("https://www.bitlinks.cn", fileConfigService.testFileConfig(id));
    }
